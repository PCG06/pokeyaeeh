...
{
    AddTextPrinterParameterized4(windowId, FONT_TINY_NARROW, x, y, 0, lineSpacing, sTextColors[colorId], 0, string);
}
...
static void PrintHeldItemName(void)
{
    ...
    x = GetStringCenterAlignXOffset(FONT_TINY_NARROW, text, 72) + 6;
   ...
   u8 monFriendship = GetMonData(&gPlayerParty[sMonSummaryScreen->curMonIndex], MON_DATA_FRIENDSHIP);
    if (moveIndex != 0)
    {
       ...
       else if (moveIndex == MOVE_FRUSTRATION)
       {
           ...
           text = gStringVar1;       }       else if (gBattleMoves[moveIndex].power < 2)
        {
